      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
        {plans.map((plan, index) => (
          ...
          >
            ...
            <div className="mb-8">
              <h3
                className={`text-2xl font-bold mb-2 ${plan.isPopular ? "text-white" : "text-[#1E293B]"}`}
              >
                {plan.name}
              </h3>
              <p
                className={`text-sm ${plan.isPopular ? "text-white/80" : "text-gray-500"}`}
              >
                {plan.description}
              </p>
            </div>
            ...
            <div className="mb-8">
              ...
              <span
                className={`text-lg font-medium ${plan.isPopular ? "text-white/80" : "text-gray-500"}`}
              >
                /Month
              </span>
            </div>
            ...
            <ul className="space-y-4 mb-10 flex-grow">
              {plan.features.map((feature, idx) => (
                <li key={idx} className="flex items-center gap-3 text-sm group">
                  <FiCheck
                    className={`text-xl flex-shrink-0 transition-transform duration-300 group-hover:scale-125 ${plan.isPopular ? "text-white" : "text-[#8B5CF6]"}`}
                  />
                  <span
                    className={
